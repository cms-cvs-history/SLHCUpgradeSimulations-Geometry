...
ModuleInfo_StrawmanA::analyze( const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  for(unsigned int i=0; i<nlayersPXB;i++){
    ...
    GeometryXLS <<"PXB"<<i+1<<" "<<pxbR_L[i]/(pxb_full_L[i]+pxb_half_L[i])<<" "<<0<<" "<<pxbZ_L[i]<<" "<<activeSurface_pxb_L[i]<<" "<<psi_pxb_L[i]<<" "<<(int)psi_pxb_L[i]*chan_per_psi<<" "<<pxb_full_L[i]+pxb_half_L[i]<<" "<<pxb_full_L[i]<<" "<<pxb_half_L[i]<<std::endl;
  }
